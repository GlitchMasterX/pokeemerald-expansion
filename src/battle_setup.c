...
const struct RematchTrainer gRematchTable[REMATCH_TABLE_ENTRIES] =
{
    ...
    [REMATCH_GABRIELLE] = REMATCH(ACADEMY_HEAD, TRAINER_GABRIELLE_2, TRAINER_GABRIELLE_3, TRAINER_GABRIELLE_4, TRAINER_GABRIELLE_5, MT_PYRE_3F),
    ...
    [REMATCH_SAWYER] = REMATCH(SPARK_GRUNT_1, TRAINER_SAWYER_2, TRAINER_SAWYER_3, TRAINER_SAWYER_4, TRAINER_SAWYER_5, MT_CHIMNEY),
    ...
    [REMATCH_NORMAN] = REMATCH(TRAINER_NORMAN_1, TRAINER_NORMAN_2, TRAINER_NORMAN_3, TRAINER_NORMAN_4, TRAINER_NORMAN_5, ASTRALIS_ACADEMY),
...
u8 BattleSetup_GetTerrainId(void)
{
    ...
    if (MetatileBehavior_IsCavedark(tileBehavior))
        return BATTLE_TERRAIN_CAVEDARK;
...
static void CB2_EndTrainerBattle(void)
{
    ...
    {
        if (InBattlePyramid() || InTrainerHillChallenge() || (!NoAliveMonsForPlayer()) || (sTrainerBattleMode == TRAINER_BATTLE_NO_WHITEOUT_CONTINUE_SCRIPT ||  sTrainerBattleMode == TRAINER_BATTLE_NO_INTRO_NO_WHITEOUT))
            SetMainCallback2(CB2_ReturnToFieldContinueScriptPlayMapMusic);
        else
            SetMainCallback2(CB2_WhiteOut);
    }
